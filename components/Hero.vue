    ...
    async checkUserRating() {
      try {
        ...
        data.forEach((row) => {
          ...
          if (row.favorites_json[favoriteType]) {
    ...
    async updateUserRating(rating) {
      ...
      try {
        ...
        if (!favoritesObject[favoriteType]) {
          throw new Error(`No favorites of type ${favoriteType} found`);
        }
    ...
    async updateUserRatingAndReview(rating, review) {
      ...
      try {
        ...
        if (itemIndex !== -1) {
          ...
          if (updateError) {
            throw new Error('Error updating rating and review:', updateError.message);
          }
