            ...
            <div class="tv-release-card__info">
              ...
              <div v-if="item.nextEpisode" class="episode-info">
                <p class="episode-details">
                  T{{ item.nextEpisode.season_number }}E{{ item.nextEpisode.episode_number }}
                </p>
                ...
                <div class="episode-plot">
                  ...
                  <p v-else class="plot-text no-plot">
                    No hay sinopsis disponible para el próximo episodio.
                  </p>
                  <button 
                  ...
                  >
                    {{ expandedPlots[item.id] ? 'MOSTRAR MENOS' : 'MOSTRAR MÁS' }}
                  </button>
                </div>
              </div>
              <div v-else class="no-episode-info">
                <p class="tv-release-card__year">{{ extractYear(getDate(item)) }}</p>
                <p class="tv-release-card__status">{{ translateStatus(item.status) || 'Estado Desconocido' }}</p>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getEpisodeTitle(item) {
      ...
      return item.name || 'Título Desconocido';
    },
    ...
    getTitle(item) {
      return item.title || item.name || 'Título Desconocido';
    },
...
.tv-release-card:hover {
  ...
  box-shadow: 15px 15px 35px rgba(31, 97, 135, 0.6);
}
