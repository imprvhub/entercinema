...
<script>
import { getTvShowsByCompanies, getFollowedProductionCompanies } from '~/api';
...
export default {
  ...
  data() {
    return {
      ...
      sortOptions: [
        { value: 'newest-releases', label: 'Fecha de estreno: Más reciente' },
        { value: 'oldest-releases', label: 'Fecha de estreno: Más antigua' },
  ...
  methods: {
      ...
      try {
        ...
        if (followedCompanies && followedCompanies.length > 0) {
          ...
          for (const company of followedCompanies) {
             const response = await getTvShowsByCompanies(company.company_id, 1);
    ...
    async loadMore(companyId) {
        ...
        try {
            ...
            const response = await getTvShowsByCompanies(companyId, nextPage);
