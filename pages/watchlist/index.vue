    ...
    async migrateMyFavorites() {
      ...
      try {
        ...
        for (const category of ['movies', 'tv']) {
          if (!favoritesObject[category]) continue;
          
          console.log(`📁 Procesando ${category}...`);
          
          for (let i = 0; i < favoritesObject[category].length; i++) {
            const item = favoritesObject[category][i];
            const key = Object.keys(item)[0];
            const details = item[key].details;
            
            if (!details.external_ids?.imdb_id) {
              const [type, id] = key.split('/');
              console.log(`🔍 Obteniendo external_ids para ${details.nameForDb}...`);
              
              try {
                const detailsResponse = await fetch(`https://api.themoviedb.org/3/${type}/${id}?api_key=${process.env.API_KEY}&append_to_response=external_ids`);
                const detailsData = await detailsResponse.json();
                
                if (detailsData.external_ids) {
                  details.external_ids = detailsData.external_ids;
                }
              } catch (err) {
                console.error(`❌ Error obteniendo detalles de ${details.nameForDb}:`, err);
                continue;
              }
            }
            
            if (details.external_ids?.imdb_id) {
              console.log(`⭐ Obteniendo rating IMDb para ${details.nameForDb}...`);
              
              try {
                const response = await fetch(`/api/imdb-rating/${details.external_ids.imdb_id}`);
                const imdbData = await response.json();
                
                if (imdbData.found) {
                  details.imdb_rating = imdbData.score;
                  details.imdb_votes = imdbData.votes;
                  details.rating_source = 'imdb';
                  totalUpdated++;
                  console.log(`✅ ${details.nameForDb}: ${imdbData.score}/10 (IMDb)`);
                } else {
                  details.rating_source = 'tmdb';
                  console.log(`ℹ️ ${details.nameForDb}: usando TMDB`);
                }
              } catch (err) {
                console.error(`❌ Error obteniendo rating de ${details.nameForDb}:`, err);
                details.rating_source = 'tmdb';
              }
            } else {
              details.rating_source = 'tmdb';
              console.log(`ℹ️ ${details.nameForDb}: sin IMDb ID, usando TMDB`);
            }
        ...
        if (updateError) {
          console.error('❌ Error guardando:', updateError);
          return;
        }
        
        console.log(`✅ ¡Migración completada! ${totalUpdated} items actualizados con ratings de IMDb`);
