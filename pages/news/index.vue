<template>
  <main class="main">
    ...
    <nav class="navbar navbar-welcome">
      <h1 class="title-primary" style="color: #8BE9FD !important; margin-top:30px; margin-bottom:10px; display:flex; justify-content: center;">Latest News</h1>
...
function onImageError(item) {
  if (item.image && item.image.includes('maxresdefault.jpg')) {
      ...
      if (item.image && item.image.includes('hqdefault.jpg')) {
         item.image = null; 
      } else if (item.image) {
         item.image = null;
      }
